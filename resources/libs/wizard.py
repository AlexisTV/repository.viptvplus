...
def cleanupBackup():
	...
	for item in sorted(folder, key=os.path.getmtime):
		...
		if os.path.isdir(item): 
			...
			list.append(base)
	list = ['--- Borrar todos ---'] + list
	selected = DIALOG.select("%s: Elige los archivos a borrar de 'MyBuilds'." % ADDONTITLE, list)
	
	if selected == -1:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Borrado Cancelado![/COLOR]" % COLOR2)
	elif selected == 0: 
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres borrar todos los archivos de la carpeta 'MisBuilds'?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLOR green]Borrar[/COLOR][/B]", nolabel="[B][COLOR red]No, Cancelar[/COLOR][/B]"):
			clearedfiles, clearedfolders = cleanHouse(xbmc.translatePath(MYBUILDS))
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Archivos Borreados: [COLOR %s]%s[/COLOR] / Carpetas:[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, clearedfiles, COLOR1, clearedfolders))
		else:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Borrado Cancelado![/COLOR]" % COLOR2)
	else:
		path = filelist[selected-1]; passed = False
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres borrar [COLOR %s]%s[/COLOR] de la carpeta 'MisBuilds'?[/COLOR]" % (COLOR2, COLOR1, list[selected]), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLOR green]Borrar[/COLOR][/B]", nolabel="[B][COLOR red]No, Cancelar[/COLOR][/B]"):
...
def clearCrash():  
	...
	if len(files) > 0:
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to delete the Crash logs?' % COLOR2, '[COLOR %s]%s[/COLOR] Files Found[/COLOR]' % (COLOR1, len(files)), yeslabel="[B][COLOR green]Remove Logs[/COLOR][/B]", nolabel="[B][COLOR red]Keep Logs[/COLOR][/B]"):
			for f in files:
				os.remove(f)
			LogNotify('[COLOR %s]Limpiar Crash Logs[/COLOR]' % COLOR1, '[COLOR %s]%s Crash Logs Borrados[/COLOR]' % (COLOR2, len(files)))
		else: LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Crash Logs Cancelado[/COLOR]' % COLOR2)
	else: LogNotify('[COLOR %s]Limpiar Crash Logs[/COLOR]' % COLOR1, '[COLOR %s]No hay Crash Logs[/COLOR]' % COLOR2)
...
def backUpOptions(type, name=""):
	exclude_dirs  = [ADDON_ID, 'cache', 'system', 'Thumbnails', 'peripheral_data', 'temp', 'MisBuilds', 'library', 'keymaps']
	exclude_files = ['Textures13.db', '.DS_Store', 'advancedsettings.xml', 'Thumbs.db', '.gitignore']
	...
	try:
		...
		return
	if type == "build":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres hacer una copia de seguridad de la build actual?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancelar Backup[/COLOR][/B]", yeslabel="[B][COLOR green]Backup Build[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Por favor pon un nombre al archivo zip %s zip" % type)
			...
			for_progress  = 0
			ITEM          = []
			if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]QUIERES INCLUIR LA CARPETA ADDON DATA?" % COLOR2, 'Esta contiene [COLOR %s]TODOS[/COLOR] los ajustes de los addons. [COLOR %s]ES RECOMENDABLE[/COLOR]Guardarlo para el buen funcionamiento de los addons.' % (COLOR1, COLOR1), '[COLOR %s]%s[/COLOR] addon_data se guardara siempre[/COLOR]' % (COLOR1, ADDON_ID), yeslabel='[B][COLOR green]Incluir data[/COLOR][/B]',nolabel='[B][COLOR red]No Incluir[/COLOR][/B]'):
		...
		if not DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Skin y Menu Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Quieres hacer una copia del skin y de los menus y submenus?[/COLOR]" % COLOR2, yeslabel="[B][COLOR green]Continuar[/COLOR][/B]", nolabel="[B][COLOR red]No, Cancelar[/COLOR][/B]"): LogNotify("Skin y Menu Backup", "Cancelado!"); return False
		if name == "":
			themename = getKeyboard("","Por favor ponle un nombre al %s zip" % type)
		...
		try:
			...
			try:
				...
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]No se ha podido crear en el directorio actual, quieres cambiarlo?[/COLOR]" % COLOR2, yeslabel="[B][COLOR green]Cambiar Directorio[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
					openS()
					return
				else:
					return
		...
		try:
				...
				if 'script.skinshortcuts' in match:
						for base, dirs, files in os.walk(os.path.join(ADDOND,'script.skinshortcuts')):
							...
							for file in files:
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Skin y Menu Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Quieres incluir las imagenes del skin de la carpeta [COLOR %s]Backgrounds[/COLOR]?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR green]Si[/COLOR][/B]", nolabel="[B][COLOR red]No, Continuar[/COLOR][/B]"):
			...
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Skin y Menu Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Quieres incluir algun addon?[/COLOR]" % (COLOR2), yeslabel="[B][COLOR green]Si[/COLOR][/B]", nolabel="[B][COLOR red]No, Continuar[/COLOR][/B]"):
				...
				for folder in sorted(fold, key = lambda x: x):
					...
					if os.path.exists(xml):
						...
						if len(match) > 0:
							...
							addonfolds.append(foldername)
				if KODIV > 16:
					selected = DIALOG.multiselect("%s: Elige los addons que quieres incluir zip." % ADDONTITLE, addonnames)
				else:
					selected = []; choice = 0
					tempaddonnames = ["-- Click here to Continue --"] + addonnames
					while not choice == -1:
						choice = DIALOG.select("%s: Elige los addons que quieres incluir zip." % ADDONTITLE, tempaddonnames)
						if choice == -1: break
						elif choice == 0: break
						else: 
							choice2 = (choice-1)
					...
					for item in selected:
						for base, dirs, files in os.walk(os.path.join(ADDONS,addonfolds[item])):
							...
							for file in files:
								...
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Skin y Menu Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Quieres incluir los ajustes de kodi [COLOR %s]guisettings.xml[/COLOR]?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR green]Si[/COLOR][/B]", nolabel="[B][COLOR red]No, Continuar[/COLOR][/B]"):
				zipf.write(GUISETTINGS, '/userdata/guisettings.xml', zipfile.ZIP_DEFLATED)
		except Exception, e:
			zipf.close()
			log("[Back Up] Type = '%s': %s" % (type, str(e)), xbmc.LOGNOTICE)
			DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] skin zip fallido:[/COLOR]" % (COLOR1, themename, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
			if not tempzipname == '':
		...
		if not tempzipname == '':
			...
			if success == 0:
				xbmcvfs.copy(tempzipname, zipname)
				xbmcvfs.delete(tempzipname)
		DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] skin zip terminado:[/COLOR]" % (COLOR1, themename, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, zipname))
	elif type == "addondata":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres hacer una copia de seguridad de la carpeta addon_data?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancelar Backup[/COLOR][/B]", yeslabel="[B][COLOR green]Backup Addon_Data[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Por favor pon un nombre al %s zip" % type)
				if not name: return False
			...
			DP.create("[COLOR %s]%s[/COLOR][COLOR %s]: Creando Zip[/COLOR]" % (COLOR1, ADDONTITLE,COLOR2), "[COLOR %s]Creaando backup zip" % COLOR2, "", "Espera...[/COLOR]")
			...
			for base, dirs, files in os.walk(ADDOND):
				...
				for file in files:
					try:
						for_progress += 1
						progress = percentage(for_progress, N_ITEM) 
						DP.update(int(progress), '[COLOR %s]Creando backup zip: [COLOR%s]%s[/COLOR] / [COLOR%s]%s[/COLOR]' % (COLOR2, COLOR1, for_progress, COLOR1, N_ITEM), '[COLOR %s]%s[/COLOR]' % (COLOR1, file), '')
						fn = os.path.join(base, file)
						if file in LOGFILES: log("[Back Up] Type = '%s': Ignore %s" % (type, file), xbmc.LOGNOTICE); continue
						elif os.path.join(base, file) in bad_files: log("[Back Up] Type = '%s': Ignore %s" % (type, file), xbmc.LOGNOTICE); continue
						elif os.path.join('addons', 'packages') in fn: log("[Back Up] Type = '%s': Ignore %s" % (type, file), xbmc.LOGNOTICE); continue
			...
			if not tempzipname == '':
				...
				if success == 0:
					...
					xbmcvfs.delete(tempzipname)
			DP.close()
			DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR] [COLOR %s]backup terminado:[/COLOR]" % (COLOR1, name, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, zipname))
...
def restoreLocal(type):
	...
	try:
		...
		DIALOG.ok(ADDONTITLE, "[COLOR %s]Error making Back Up directories:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
		return
	file = DIALOG.browse(1, '[COLOR %s]Elige el archivo que quieres restaurar[/COLOR]' % COLOR2, 'files', '.zip', False, False, mybuilds)
	log("[RESTORE BACKUP %s] File: %s " % (type.upper(), file), xbmc.LOGNOTICE)
	if file == "" or not file.endswith('.zip'):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Restauracion: Cancelado[/COLOR]" % COLOR2)
		return
	DP.create(ADDONTITLE,'[COLOR %s]Instalando Backup' % COLOR2,'', 'Espera[/COLOR]')
	...
	try:
		...
		DP.update(0, '', 'Copiando archivos al paquete: Completado')
		zipfile.ZipFile(file, 'r')
	...
	if int(errors) >= 1:
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, fn), 'Completado: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Quieres ver los errores?[/COLOR]', nolabel='[B][COLOR red]No[/COLOR][/B]',yeslabel='[B][COLOR green]Ver Errores[/COLOR][/B]')
		if yes:
...
def restoreExternal(type):
	source = DIALOG.browse(1, '[COLOR %s]Elige el archivo que quieres restaurar[/COLOR]' % COLOR2, 'files', '.zip', False, False)
	if source == "" or not source.endswith('.zip'):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Restauracion Externa: Cancelada[/COLOR]" % COLOR2)
		return
	if not source.startswith('http'):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Restauracion Externa: URL Invalida[/COLOR]" % COLOR2)
		return
	try: 
		work = workingURL(source)
	except:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Restauracion externa: Error Valid URL[/COLOR]" % COLOR2)
	...
	DP.create(ADDONTITLE,'[COLOR %s]Descargando Zip file' % COLOR2,'', 'Espera[/COLOR]')
	if type == "gui": loc = USERDATA
	...
	lookandFeelData('save')
	if int(errors) >= 1:
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, zname), 'Completado: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Would you like to view the errors?[/COLOR]', nolabel='[B][COLOR red]No Thanks[/COLOR][/B]',yeslabel='[B][COLOR green]View Errors[/COLOR][/B]')
		if yes:
			TextBox(ADDONTITLE, error.replace('\t',''))
	setS('installed', 'true')
	setS('extract', str(percent))
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Borrar Paquetes" % COLOR2, "[COLOR %s]%s[/COLOR] No hay / [COLOR %s]%s[/COLOR] en peso." % (COLOR1, str(file_count), COLOR1, size), "Quieres borrarlos[/COLOR]", nolabel='[B][COLOR red]No borrar[/COLOR][/B]',yeslabel='[B][COLOR green]Borrar Paquetes[/COLOR][/B]')
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Borrar Paquetes: Hecho![/COLOR]' % COLOR2)
				else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Borrar Paquetes: No hay![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Borrar Paquetes: Error![/COLOR]' % COLOR2)
...
def clearPackagesStartup():
	...
	if os.path.exists(PACKAGES):
		...
		try:
			...
			if file_count > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Paquetes: Hecho: %s[/COLOR]' % (COLOR2, convertSize(cleanupsize)))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Paquetes: No hay![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Paquetes: Error![/COLOR]' % COLOR2)
			log("Clear Packages Error: %s" % str(e), xbmc.LOGERROR)
...
def checkSources():
	if not os.path.exists(SOURCES):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Archivo Sources.xml no encontrado![/COLOR]" % COLOR2)
		return False
	x      = 0
	bad    = []
	remove = []
	...
	if len(match) > 0:
		...
		DP.create(ADDONTITLE, "[COLOR %s]Escaneando links rotos en fuentes de contenido[/COLOR]" % COLOR2)
		...
		if len(bad) > 0:
			choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] Fuente(s) Encontrada rota" % (COLOR1, len(bad), COLOR2),"Quieres borrarlas todas o elegir una por una?[/COLOR]", yeslabel="[B][COLOR green]Borrar todas[/COLOR][/B]", nolabel="[B][COLOR red]Elegir cuales borrar[/COLOR][/B]")
			if choice == 1:
				remove = bad
			else:
				for name, path, sharing, working in bad: 
					log("%s sources: %s, %s" % (name, path, working), xbmc.LOGNOTICE)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] ha sido reportada" % (COLOR1, name, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLOR green]Borrar fuente[/COLOR][/B]", nolabel="[B][COLOR red]Mantener Fuente[/COLOR][/B]"):
			...
			if len(remove) > 0:
				...
				removed = len(remove)
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Comprobar fuentes completado" % COLOR2, "Trabajando: [COLOR %s]%s[/COLOR] | Mantenidas: [COLOR %s]%s[/COLOR] | Borradas: [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR2, COLOR1, alive, COLOR1, kept, COLOR1, removed))
			else: log("No Bad Sources to be removed.", xbmc.LOGNOTICE)
		else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Todas las fuentes funcionan[/COLOR]" % COLOR2)
...
def checkRepos():
	...
	if len(repolist) == 0:
		DP.close()
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Repositorios no encontrados![/COLOR]" % COLOR2)
		return
	...
	if len(badrepos) > 0:
		msg  = "[COLOR %s]A continuacion hay una lista de repositorios que no se repararon. Esto no significa que esten rotos, a veces los hosts caen por un corto periodo de tiempo. Haz una serie de escaneos de su lista de repositorios antes de eliminar un repositorio solo para asegurarte de que esta roto.[/COLOR][CR][CR][COLOR %s]" % (COLOR2, COLOR1)
		msg += '[CR]'.join(badrepos)
		msg += '[/COLOR]'
		TextBox("%s: Bad Repositories" % ADDONTITLE, msg)
	else: 
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Todos los repositorios funcionan![/COLOR]" % COLOR2)
...
def killxbmc(over=None):
	if over: choice = 1
	else: choice = DIALOG.yesno('Forzar cierre de Kodi', '[COLOR %s]Estas a punto de cerrar Kodi' % COLOR2, 'Quieres continuar?[/COLOR]', nolabel='[B][COLOR red] No, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Forzar Cierre de Kodi[/COLOR][/B]')
	if choice == 1:
...
def reloadFix(default=None):
	DIALOG.ok(ADDONTITLE, "[COLOR %s]ATENCION: A veces recargar el Perfil puede causar el cierre de Kodi.  Mientras Kodi esta recargando el Perfil, POR FAVOR, NO PULSES NINGUN BOTON![/COLOR]" % COLOR2)
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	if default == None:
		lookandFeelData('save')
	redoThumbs()
...
def purgeDb(name):
	...
	textexe.close()
	log('%s Purga DB completada.' % name, xbmc.LOGNOTICE)
	show = name.replace('\\', '/').split('/')
	LogNotify("[COLOR %s]Purgar Database[/COLOR]" % COLOR1, "[COLOR %s]%s Completa[/COLOR]" % (COLOR2, show[len(show)-1]))
...
def oldThumbs():
	...
	if len(images) > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar Imagenes: %s Archivos / %s MB[/COLOR]!' % (COLOR2, str(len(images)), removed))
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Borrar imagenes: No hay![/COLOR]' % COLOR2)
